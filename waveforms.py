...
class WaveFormModel(ABC):
    ...
    def __init__(self, objType, fcutPar, is_newtonian=False, is_tidal=False, is_HigherModes=False, is_chi1chi2=True, is_Precessing=False, is_LAL=False, is_prec_ang=False, is_eccentric=False, is_holomorphic=False, is_s0Diss=False, apply_fcut=True):
        """
        Constructor method
        """
        ...
        ###Zihan: add class variable here
        self.is_s0Diss = is_s0Diss
        ###
...
##############################################################################
# IMRPhenomD + Dissipation WAVEFORM
##############################################################################
        
class IMRPhenomD_Diss(WaveFormModel):
    """
    IMRPhenomD_Diss waveform model.
    ...
    Relevant references:
        ...
        [3] `arxiv:2404.14641 <https://arxiv.org/abs/2404.14641>`_
    ...
    def __init__(self, fRef=None, **kwargs):
        ...
        super().__init__('BBH', fcutPar, is_tidal=True, is_s0Diss=True, **kwargs)
    ...
    def Phi(self, f, **kwargs):
        ...
        #Phi_TDN_cut= np.where(fgrid < f_tape, Phi_TDN, Phi_TDN[int(f_tape[0])])
        ###
        ...
        if self.apply_fcut:
            ...
            ### Zihan: add Phi_TDN here:
            return phis + Phi_TDN + np.where(fgrid < self.fcutPar, - t0*(fgrid - fRef) - phiRef, 0.)
            ###
        else:
            phiRef = np.where(fRef < self.PHI_fJoin_INS, PhiInspcoeffs['initial_phasing'] + PhiInspcoeffs['two_thirds']*(fRef**(2./3.)) + PhiInspcoeffs['third']*(fRef**(1./3.)) + PhiInspcoeffs['third_log']*(fRef**(1./3.))*np.log(np.pi*fRef)/3. + PhiInspcoeffs['log']*np.log(np.pi*fRef)/3. + PhiInspcoeffs['min_third']*(fRef**(-1./3.)) + PhiInspcoeffs['min_two_thirds']*(fRef**(-2./3.)) + PhiInspcoeffs['min_one']/fRef + PhiInspcoeffs['min_four_thirds']*(fRef**(-4./3.)) + PhiInspcoeffs['min_five_thirds']*(fRef**(-5./3.)) + (PhiInspcoeffs['one']*fRef + PhiInspcoeffs['four_thirds']*(fRef**(4./3.)) + PhiInspcoeffs['five_thirds']*(fRef**(5./3.)) + PhiInspcoeffs['two']*fRef*fRef)/eta, np.where(fRef<fMRDJoin, (beta1*fRef - beta3/(3.*fRef*fRef*fRef) + beta2*np.log(fRef))/eta + C1Int + C2Int*fRef, (-(alpha2/fRef) + (4.0/3.0) * (alpha3 * (fRef**(3./4.))) + alpha1 * fRef + alpha4 * np.arctan((fRef - alpha5 * fring)/fdamp))/eta + C1MRD + C2MRD*fRef))
        ...
        ### Zihan: add Phi_TDN here:
        return phis + Phi_TDN - t0*(fgrid - fRef) - phiRef
        ###
...
##############################################################################
# IMRPhenomD_NRTidalv2_Diss WAVEFORM, JS_EDIT
##############################################################################

class IMRPhenomD_NRTidalv2_Diss(WaveFormModel):
    """
    IMRPhenomD_NRTidalv2 waveform model.
    ...
    def __init__(self, fRef=None, **kwargs):
        ...
        super().__init__('BNS', fcutPar, is_tidal=True, is_s0Diss=True, **kwargs)
    ...
    def Phi(self, f, **kwargs):
        ...
        SS_3p5PN  = - 400.*np.pi*(QuadMon1-1.)*chi12*m1ByM*m1ByM - 400.*np.pi*(QuadMon2-1.)*chi22*m2ByM*m2ByM
        SSS_3p5PN = 10.*((m1ByM*m1ByM+308./3.*m1ByM)*chi1+(m2ByM*m2ByM-89./3.*m2ByM)*chi2)*(QuadMon1-1.)*m1ByM*m1ByM*chi12 + 10.*((m2ByM*m2ByM+308./3.*m2ByM)*chi2+(m1ByM*m1ByM-89./3.*m1ByM)*chi1)*(QuadMon2-1.)*m2ByM*m2ByM*chi22 - 440.*OctMon1*m1ByM*m1ByM*m1ByM*chi12*chi1 - 440.*OctMon2*m2ByM*m2ByM*m2ByM*chi22*chi2
        if self.apply_fcut: ###JS_EDIT: add Phi_TDN to existing waveform in the same way.
            return phis + Phi_TDN + np.where(fgrid < self.fcutPar, - t0*(fgrid - fRef) - phiRef + tidal_phase + (SS_3p5PN + SSS_3p5PN)*TF2OverallAmpl*((np.pi*fgrid)**(2./3.)), 0.)
        else:
            return phis + Phi_TDN - t0*(fgrid - fRef) - phiRef + tidal_phase + (SS_3p5PN + SSS_3p5PN)*TF2OverallAmpl*((np.pi*fgrid)**(2./3.))
