...
def phase_qdol(f, theta, fix_bh_superradiance, EBdual):
    """
    ...
    hs_E = (h1s1_E*m1**3 + h2s1_E*m2**3) / M**3
    ha_E = (h1s1_E*m1**3 - h2s1_E*m2**3) / M**3
    hs_B = (h1s1_B*m1**3 + h2s1_B*m2**3) / M**3
    ha_B = (h1s1_B*m1**3 - h2s1_B*m2**3) / M**3
    
    
    # 3.5PN Dissipation (Cubic spin)
    hs3_E = (h1s3_E*m1**3 + h2s3_E*m2**3) / M**3
    ...
    if fix_bh_superradiance:
        ...
        h0 = (-2.0 * h1s1_E * m1 ** 4 - 2.0 * h2s1_E * m2 ** 4) / M**4 
    else:
    ...
    psi_TDN_S_35PN += (225.0 / 8.0 * hs_B + 102975.0 / 448.0 * hs_E + 1425.0 / 16 * eta * hs_E + 675.0 / 32.0 * delta * ha_E
                       ) * chi_s 
    psi_TDN_S_35PN += (225.0 / 8.0 * ha3_B) * chi_a**3 
    psi_TDN_S_35PN += (225.0 / 8.0 * hs3_B) * chi_s**3
    psi_TDN_S_35PN += (675.0 / 8.0 * ha3_B) * chi_a * chi_s**2 
    psi_TDN_S_35PN += (675.0 / 8.0 * hs3_B) * chi_a**2 * chi_s 
    psi_TDN_S_35PN *= v7
...
def PN4_pp_phase(f, theta):
    """
    
    Computes the 4PN TaylorF2 phase of with point particle contributions. 
    ...
    Returns
    -------
    phase (array): 4PN phase as a function of frequency
...
def gen_h0_qdol_phase(f, theta_more, f_ref, alpha=1.0,fix_bh_superradiance=True, EBdual=True):
    """
    ...
    TDN_Phi_tape = phase_qdol(f_tape, theta_more,fix_bh_superradiance,EBdual)
        
    TDN_Phi_cut= jnp.where(f < f_tape, TDN_Phi, TDN_Phi_tape)

    
    TDN_Phi_ref = phase_qdol(f_ref, theta_more,fix_bh_superradiance,EBdual)
    TDN_Phi_cut -=  TDN_Phi_ref
